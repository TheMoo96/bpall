...
To buy a component is not a trivial task as a beginner. Decision fatigue sets in real quick when the pool of choices is in the thousands for a simple resistor. Even if you already know the value, the choice of how to mount it, which size it has, the tolerance of its value, and probably something else still remains.

For the pomoljus I chose to go with *surface mounted components* (SMDs), as *through hole components* (THTs) require a hole... for the component... to be mounted through... Which takes up space on the other side, as well as the side you're mounting it on. While THTs are easier to solder, they do end up taking up much more space. As I wished for the pomoljus to be a small device, I chose to go for SMDs instead. For SMDs, you're either limited by the wattage requirements (which for this project are low enough), or your proficiency with a soldering iron. You could go for a resistor the size of a grain of sand, or you could go for something that doesn't drift across the room on the air-streams created by the heat radiating off of your sweating forehead as you try recover from a sudden desktop visit from your cat... ... I chose 0608, for my resistors, and the rest of the components were around that size too. For the transistor you also have the choice of package, which is basically the same story but you're instead challenged by the pitch (or size) of the leads coming off the component. 

### Where

Of course your selection is only as good as the store you use. Here it's worth leaning towards more industrial-focused stores, such as Mouser, Elfa Distrelec, DigiKey, Farnell, or TME. [add links] Sometimes you can get all your components from one place, other times you gotta shop around. I stumbled upon TME when they seemed to be the only store who carried the SK6812, at a reasonable price. They delivered in a silver antistatic baggie containing the bubble wrapped components, a huge silica baggie, and a moisture indicator. Pros all around. Just be mindful to not buy them from a store which adds a 40% markup for the same product simply because they're *easy* to buy from (looking at you Electrokit).

## The CAD

When it comes to the making of the CAD, you have two steps. The first step is to make the abstracted electrical diagram, which allows you to put the entire project together without pesky restrictions such as *"physics"* getting in your way. This is then followed by creating the layout of the PCB, but now with physics getting *very much* in your way.
...
2. https://hackaday.com/2017/01/25/ws2812b-led-clones-work-better-than-originals/
